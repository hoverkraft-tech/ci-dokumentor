...
export class ExamplesSectionGenerator extends GitHubActionsSectionGeneratorAdapter {
  ...
  private generateActionExamples(
    formatterAdapter: FormatterAdapter,
    action: GitHubAction
  ): Buffer {
    ...
    if (action.inputs && Object.keys(action.inputs).length > 0) {
      ...
      Object.entries(action.inputs)
        ...
        .forEach(([key, input]) => {
          const value =
            input.default ||
            (input.type === 'boolean' ? 'true' : 'example-value');
          basicExample += `\n    ${key}: ${value}`;
        });
    }
  ...
  with:`;
      ...
      Object.entries(action.inputs).forEach(([key, input]) => {
        const value =
          input.default ||
          (input.type === 'boolean' ? 'false' : 'custom-value');
        advancedExample += `\n    ${key}: ${value}`;
      });
...
- name: Use outputs
  ...
  private generateWorkflowExamples(
    formatterAdapter: FormatterAdapter,
