...
`,
      ...
      it.each(generateSectionScenarios)("$name", async ({
        manifest,
        version,
        destinationContent,
        expectedContent,
...
<!-- examples:end -->`),
    ...
    it("should throws on file system errors", async () => {
      ...
      mockReaderAdapter.resourceExists.mockImplementation(() => {
        throw new Error("EACCES, permission denied '/test/repo/examples'");
      });
