...
`);
    ...
    it('should generate the same section regardless of manifest type', async () => {
      ...
      // Act
      const actionResult = await generator.generateSection({ formatterAdapter, manifest: actionManifest, repositoryProvider: mockRepositoryProvider });
      const workflowResult = await generator.generateSection({ formatterAdapter, manifest: workflowManifest, repositoryProvider: mockRepositoryProvider });
...
`);
    ...
    it('should generate the same section regardless of repository', async () => {
      ...
      // Act
      const result1 = await generator.generateSection({ formatterAdapter, manifest, repositoryProvider: mockRepositoryProvider });
      const result2 = await generator.generateSection({ formatterAdapter, manifest, repositoryProvider: alternativeRepositoryProvider });
