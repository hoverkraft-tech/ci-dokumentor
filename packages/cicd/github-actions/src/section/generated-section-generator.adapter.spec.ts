...
`);
    ...
    it("should generate the same section regardless of manifest type", async () => {
      ...
      // Act
      const actionResult = await generator.generateSection({
        formatterAdapter,
        manifest: actionManifest,
        repositoryProvider: mockRepositoryProvider,
        destination: "README.md",
      });
      const workflowResult = await generator.generateSection({
        formatterAdapter,
...
`);
    ...
    it("should generate the same section regardless of repository", async () => {
      ...
      const alternativeRepositoryProvider =
        RepositoryProviderMockFactory.create({
          getRepositoryInfo: RepositoryInfoMockFactory.create({
      ...
      // Act
      const result1 = await generator.generateSection({
        formatterAdapter,
        manifest,
        repositoryProvider: mockRepositoryProvider,
        destination: "README.md",
      });
      const result2 = await generator.generateSection({
        formatterAdapter,
        manifest,
        repositoryProvider: alternativeRepositoryProvider,
        destination: "README.md",
      });
