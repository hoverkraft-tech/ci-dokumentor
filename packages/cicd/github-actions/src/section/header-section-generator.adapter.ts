...
@injectable()
export class HeaderSectionGenerator extends HeaderSectionMixin<
...
>(GitHubActionsSectionGeneratorAdapter) {
  public override generateTitle(
    ...
    const title = this.getTitlePrefix(manifest).append(
      this.getDisplayName(manifest, repositoryInfo),
    );
  ...
  private generateBrandingIcon(
    ...
    const data = new ReadableContent(
      `data:image/svg+xml;base64,${Buffer.from(featherIcon).toString("base64")}`,
    );
