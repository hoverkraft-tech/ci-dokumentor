...
`,
      ...
      it.each(cases)("should generate usage section for $title", async ({
        inputs,
        version,
        expected,
      }) => {
        // Arrange
        const manifest: GitHubAction = inputs
          ? GitHubActionMockFactory.create({ inputs })
          : GitHubActionMockFactory.create();
