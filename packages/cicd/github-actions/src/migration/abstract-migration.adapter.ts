...
export abstract class AbstractMigrationAdapter implements MigrationAdapter {
  ...
  protected abstract migrateContent(
   ...
   */
  protected processMarkerMappings(
    content: ReadableContent,
    formatterAdapter: FormatterAdapter,
  ): ReadableContent {
    ...
    const makeNonGlobal = (r?: RegExp) =>
      r ? new RegExp(r.source, r.flags.replace(/g/g, "")) : undefined;
    ...
    const patternsIdentical =
      startPattern &&
      endPattern &&
      startPattern.source === endPattern.source &&
      startPattern.flags === endPattern.flags;
   ...
   */
  private mergeConsecutiveSections(
    content: ReadableContent,
    formatterAdapter: FormatterAdapter,
  ): ReadableContent {
    let result = content;
  ...
  private addMissingSections(
    ...
    const findInsertAfterSection = (
      content: ReadableContent,
      sectionName: SectionIdentifier,
    ): number => {
