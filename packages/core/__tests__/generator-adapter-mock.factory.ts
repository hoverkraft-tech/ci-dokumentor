...
type GeneratorAdapterDefaults = Partial<{
  ...
  generateDocumentation: Awaited<
    ReturnType<GeneratorAdapter["generateDocumentation"]>
  >;
}>;
...
export class GeneratorAdapterMockFactory {
  static create(defaults?: GeneratorAdapterDefaults): Mocked<GeneratorAdapter> {
    const mock = {
      ...
      getSupportedSections: vi.fn() as Mocked<
        GeneratorAdapter["getSupportedSections"]
      >,
      supportsSource: vi.fn() as Mocked<GeneratorAdapter["supportsSource"]>,
      getDocumentationPath: vi.fn() as Mocked<
        GeneratorAdapter["getDocumentationPath"]
      >,
      generateDocumentation: vi.fn() as Mocked<
        GeneratorAdapter["generateDocumentation"]
      >,
    ...
    if (defaults?.generateDocumentation !== undefined) {
      mock.generateDocumentation.mockResolvedValue(
        defaults.generateDocumentation,
      );
    }
