...
export class RepositoryServiceMockFactory {
  static create(defaults?: RepositoryServiceDefaults): Mocked<RepositoryService> {
    const mock = {
      ...
      >,
      getRepositoryProviderByPlatform: vi.fn() as Mocked<
        RepositoryService["getRepositoryProviderByPlatform"]
      >,
      getSupportedRepositoryPlatforms: vi.fn() as Mocked<
    ...
    if (defaults?.getRepositoryProviderByPlatform !== undefined) {
      mock.getRepositoryProviderByPlatform.mockReturnValue(
        defaults.getRepositoryProviderByPlatform,
      );
    }
    if (defaults?.getSupportedRepositoryPlatforms !== undefined) {
      mock.getSupportedRepositoryPlatforms.mockReturnValue(
        defaults.getSupportedRepositoryPlatforms,
      );
    }
