...
export interface FormatterServiceDefaults {
  getFormatterAdapterForFile: ReturnType<
...
export class FormatterServiceMockFactory {
  static create(
    defaults?: Partial<FormatterServiceDefaults>,
  ): Mocked<FormatterService> {
    const mock = {
      getFormatterAdapterForFile: vi.fn() as Mocked<
        FormatterService["getFormatterAdapterForFile"]
      >,
    } as Mocked<FormatterService>;

    if (defaults?.getFormatterAdapterForFile !== undefined) {
      mock.getFormatterAdapterForFile.mockReturnValue(
        defaults.getFormatterAdapterForFile,
      );
    }
