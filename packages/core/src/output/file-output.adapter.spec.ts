...
describe('FileOutputAdapter', () => {
    ...
    describe('writeSection', () => {
        ...
        it('should replace existing section content when section already exists', async () => {
            // Arrange
            const adapterWithExistingSection = new FileOutputAdapter('/test/existing-section.md', formatter);
        ...
        it('should handle empty files correctly', async () => {
            // Arrange
            const emptyFileAdapter = new FileOutputAdapter('/test/empty.md', formatter);
        ...
        it('should handle multiple sections without affecting other sections', async () => {
            // Arrange
            const multipleSectionsAdapter = new FileOutputAdapter('/test/multiple-sections.md', formatter);
        ...
        it('should handle sections with complex content including line breaks', async () => {
            // Arrange
            const complexData = Buffer.from('Line 1\nLine 2\n\nLine 4 with spaces   \n<!-- embedded comment -->');
        ...
        it('should preserve file content order when adding new section', async () => {
            ...
            const sectionStartIndex = lines.findIndex(line => line.includes(`<!-- ${testSectionIdentifier}:start -->`));
            expect(sectionStartIndex).toBeGreaterThan(1);
        });

        it('should handle non-existent file by creating it', async () => {
            // Arrange
            const newFileAdapter = new FileOutputAdapter('/test/new-file.md', formatter);
    ...
    describe('error handling', () => {
        it('should handle file system errors gracefully', async () => {
            ...
            const readOnlyAdapter = new FileOutputAdapter('/readonly/document.md', formatter);

            // Act & Assert
            await expect(readOnlyAdapter.writeSection(testSectionIdentifier, testData))
                .rejects.toThrow();
        });

        it('should handle invalid file paths', async () => {
            // Arrange
            const invalidPathAdapter = new FileOutputAdapter('/nonexistent/path/file.md', formatter);

            // Act & Assert
            await expect(invalidPathAdapter.writeSection(testSectionIdentifier, testData))
                .rejects.toThrow();
        });
    });
    ...
    describe('integration scenarios', () => {
        it('should handle multiple consecutive writes to the same section', async () => {
            ...
            const startMarkers = (fileContent.match(new RegExp(`<!-- ${testSectionIdentifier}:start -->`, 'g')) || []).length;
            const endMarkers = (fileContent.match(new RegExp(`<!-- ${testSectionIdentifier}:end -->`, 'g')) || []).length;
