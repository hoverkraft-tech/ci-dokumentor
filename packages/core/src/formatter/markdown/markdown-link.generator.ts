...
export class MarkdownLinkGenerator {
  ...
  private replaceUrl(
    ...
    return fullLinkFormat
      ? `[${cleanUrl}](${cleanUrl})${trailingPunct}`
      : `<${cleanUrl}>${trailingPunct}`;
  }

  private splitIntoCodeAndTextSegments(
    content: ReadableContent,
  ): Array<{ code: boolean; content: ReadableContent }> {
    ...
    while (idx < contentLength) {
      ...
      if (content.includesAt("`", idx)) {
        ...
        segments.push({
          code: true,
          content: new ReadableContent(content.slice(idx, end + tickCount)),
        });
        idx = end + tickCount;
        continue;
      }
