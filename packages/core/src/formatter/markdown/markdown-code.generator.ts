...
export class MarkdownCodeGenerator {
  ...
  backtickFenceFor(content: ReadableContent): ReadableContent {
    if (content.isEmpty()) {
      return ReadableContent.empty().padEnd(
        MarkdownCodeGenerator.MIN_FENCE_LEN,
   ...
   */
  public findInlineCode(
    content: ReadableContent,
    existingBlocks: Array<{ start: number; end: number }>,
  ): Array<{ start: number; end: number; delimLen: number }> {
  ...
  public findCodeBlocks(content: ReadableContent): Array<{
    start: number;
    end: number;
    innerStart: number;
    innerEnd: number;
    ...
    while (pos < contentSize) {
      ...
      if (backtickIdx === -1 && tildeIdx === -1) {
        ...
        idx = backtickIdx;
        marker = MarkdownCodeGenerator.TICK_CHAR_CODE;
      ...
      if (infoLineEnd > k) {
        ...
        while (
          le >= ls &&
          (content.includesAt(ReadableContent.SPACE_CHAR_CODE, le) ||
            content.includesAt(ReadableContent.CARRIAGE_RETURN_CHAR_CODE, le))
        ) {
          le--;
        }
        if (le >= ls) {
