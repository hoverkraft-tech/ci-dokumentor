  ...
  private splitMultilineCell(content: ReadableContent): ReadableContent[] {
    ...
    for (const b of fenced) {
      codeBlocks.push({
        ...
        replacement: `__CODEBLOCK_${codeBlocks.length}__`,
      });
    }
    ...
    const order = codeBlocks
      .map((_, idx) => idx)
      .sort((a, b) => codeBlocks[a].start - codeBlocks[b].start);
    for (let k = order.length - 1; k >= 0; k--) {
      ...
      modifiedStr = modifiedStr
        ...
        .append(modifiedStr.slice(block.end));
    }
