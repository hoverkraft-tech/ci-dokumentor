...
describe('FormatterService', () => {
    ...
    beforeEach(() => {
        ...
        const mockMarkdownAdapter: FormatterAdapter = {
            supportsLanguage: vi.fn((language: FormatterLanguage) => language === FormatterLanguage.Markdown),
    ...
    describe('getFormatterAdapterForFile', () => {
        it('should return markdown adapter for .md file', () => {
            ...
            expect(mockFormatterAdapters[0].supportsLanguage).toHaveBeenCalledWith(FormatterLanguage.Markdown);
        });
        ...
        it('should return first matching adapter when multiple adapters support the language', () => {
            ...
            vi.mocked(mockFormatterAdapters[0].supportsLanguage).mockReturnValue(false);
            vi.mocked(mockFormatterAdapters[1].supportsLanguage).mockReturnValue(false);

            // Act & Assert
            expect(() => formatterService.getFormatterAdapterForFile(filePath))
        ...
        it('should check adapters in order and return first match', () => {
            ...
            vi.mocked(mockFormatterAdapters[1].supportsLanguage).mockReturnValue(true);
            ...
            expect(mockFormatterAdapters[0].supportsLanguage).toHaveBeenCalledWith(FormatterLanguage.Markdown);
            expect(mockFormatterAdapters[1].supportsLanguage).toHaveBeenCalledWith(FormatterLanguage.Markdown);
        });
    });
    ...
    describe('constructor', () => {
        it('should initialize with empty adapters array', () => {
            ...
            // Assert
            expect(() => service.getFormatterAdapterForFile('test.md'))
                .toThrow('No formatter adapter found for file: test.md');
        });
